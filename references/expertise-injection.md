...
## OpenClaw + Claude Code + Codex Integration Notes

### Agent Model Routing (from our stack)

| Agent | Recommended Model | Why | OpenClaw Alias |
|-------|------------------|-----|----------------|
| Orchestrator | Claude Opus | Deepest reasoning, best design judgment | `opus` |
| Research Agent | Gemini 3 Pro | Live web access for reference scouting | `gemini` |
| Validation Agent | Claude Opus | Critical reasoning for feasibility checks | `opus` |
| Agent-A (Foundation) | Claude Sonnet 4.6 | Fast structured code output | `sonnet46` |
| Agent-B (Motion) | Claude Sonnet 4.6 | Knows GSAP patterns well | `sonnet46` |
| Agent-C (UI/UX) | Claude Sonnet 4.6 | Strong CSS component patterns | `sonnet46` |
| Agent-D (Artifacts) | N/A (tool-dependent) | Runs Nano Banana Pro / Leonardo tools | — |
| Agent-E (QA) | Claude Opus | Best at adversarial review | `opus` |

### 52 Best Practices Applied to Iron Canvas

From "52 Weeks of Claude Code Lessons in 45 Minutes" (Avthar Sewrathan):
...
6. **Build in Phases (#27):** v1 (Foundation) → v2 (Motion) → v3 (Components) → v4 (Artifacts).
7. **Verify at Higher Abstraction (#31):** Don't review code line-by-line — check if the experience WORKS.
8. **Give AI a Way to Verify (#32):** Agent-E IS the verification mechanism.
9. **Auto-Document (#35):** Design PRD serves as living documentation of all decisions.
10. **Multi-Clauding with Worktrees (#38):** SWARM mode = parallel agents in isolated workspaces.
11. **Ralph Wiggum Loops (#42):** Agent-B scroll engine can iterate autonomously (build → test → fix → repeat). But NOT for taste decisions (#43).
12. **Don't Use Ralph Wiggum for Taste (#43):** Phase 7 Taste Test requires human judgment. Always.
